...
```{r}
write.csv(PALS_SALES_filtered, "PALS_SALES.csv")
```
...
```{r}
PALS_SALESfit <- cfa(latents.model1, data = PALS_SALES, se = "robust", test = "Satorra-Bentler") #robust ML fit
summary(PALS_SALESfit,fit.measures = TRUE) #to view summary of the fit
...
User Model versus Baseline Model:
  ...
  Model is unacceptable, even with the robust analysis 
...
```{r}
PALS_SALESfit2 <- cfa(latents.model2, data = PALS_SALES, se = "robust", test = "Satorra-Bentler") #robust ML fit
...
```{r}
PALS_SALESfit3 <- cfa(latents.model3, data = PALS_SALES, se = "robust", test = "Satorra-Bentler") #robust ML fit
...
Root Mean Square Error of Approximation:
  ...
  The robust fit is slightly better..but not accepatable
...
#Model5, 6 factors SALES as one unit, PALS divided into Perf.Goals and MG and SE

```{r}
latents.model6 <- 'SALES =~ SQ08 + SQ07 + SQ09 + SQ10 + SQ11 + SQ12 + SQ13 + SQ14 + SQ18 + SQ15 + SQ16 + SQ17 + SQ19 + SQ20 + SQ21 + SQ22 + SQ24 + SQ23 + SQ25 + SQ26 + SQ27 + SQ28 + SQ29 + SQ31 + SQ32
PerfGoals =~ PQ09 + PQ06 + PQ07 + PQ08 + PQ10 + PQ14 + PQ11 + PQ12 + PQ13 
MG_SE_P  =~ PQ16 + PQ15 + PQ17 + PQ18 + PQ19 + PQ02 + PQ01  + PQ03 + PQ04 + PQ05'
```

```{r}
PALS_SALESfit6  <- cfa(latents.model6 , data = PALS_SALES,se = "robust", test = "Satorra-Bentler")#stardard fit
...
Removing SE_S, the fit did not improve. it worsened. 

```{r}
latents.model7 <- 'SALES =~ SQ18 + SQ15 + SQ16 + SQ17 + SQ19 + SQ20 + SQ21 + SQ22 + SQ24 + SQ23 + SQ25 + SQ26 + SQ27 + SQ28 + SQ29 + SQ31 + SQ32
PerfGoals =~ PQ09 + PQ06 + PQ07 + PQ08 + PQ10 + PQ14 + PQ11 + PQ12 + PQ13 
MG_SE_P  =~ PQ16 + PQ15 + PQ17 + PQ18 + PQ19 + PQ02 + PQ01  + PQ03 + PQ04 + PQ05'
```

```{r}
PALS_SALESfit7  <- cfa(latents.model7 , data = PALS_SALES,se = "robust", test = "Satorra-Bentler")#stardard fit
summary(PALS_SALESfit7) #to view summary of the fit
vartable(PALS_SALESfit7 ) # to have the summary of variables converted to a dataframe
parameterestimates(PALS_SALESfit7 ) #parameterEstimates() includes the estimates, standard errors, z value, p value, and 95% CIs for all model parameters
summary(PALS_SALESfit7  ,fit.measures = TRUE)

```

User Model versus Baseline Model:

  Comparative Fit Index (CFI)                    0.800       0.811
  Tucker-Lewis Index (TLI)                       0.787       0.799
                                                                  
 RMSEA                                          0.077       0.062
```{r}
sink("model7.doc")
print(summary(PALS_SALESfit7  ,fit.measures = TRUE))
sink()
```

```{r}
modindices(PALS_SALESfit7, sort. = T)
```
