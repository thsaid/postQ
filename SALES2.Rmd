...
24/3 (full referts to the version that has no missing values)

```{r}
SALES <- read.csv("SALES_full.csv")
```
...
```{r}
imputed_Data <- mice(SALES, m=5, maxit = 5, method = 'pmm', seed = 5) #m  – Refers to 5 imputed data sets (default is 5), maxit –  Refers to no. of iterations taken to impute missing values, deafult is 5. Seed: An integer that is used as argument by the set.seed() for offsetting the random number generator. Default is to leave the random number generator alone. method – Refers to method used in imputation. we used predictive mean matching 'pmm'
```

checking for the imputed values for Q25
```{r}
imputed_Data$imp$Q25 #these command shows the imputed data for Q04 for all the versions (datasets)
```


Choosing dataset no.3

```{r}
SALES <- complete(imputed_Data,3)
```

Saving SALES as a full version (or complete) 

```{r}
write.csv(SALES, "SALES_full.csv")
```
...
By comparing the mean of Q25 before and after the imputation, it appears that it has not changed from 4.149(before imputation) to 4.15(after imputation)
...
```{r}
library(tidyverse)
SALES <- cbind(SALES, total = rowSums(Filter(is.numeric, SALES), na.rm = TRUE))
```


```{r}
describeBy(SALES,"Gender")
```
...
It appears that the score distribution is not normal, there is a left tail and the score is skewed. Kurtosis is 6.6 and skewness is -1.65



```{r}
round(stat.desc(SALES$total, basic = FALSE, norm = TRUE), digits= 3)
```

```{r}
sink("SALES_Shapiro.docx")
print(round(stat.desc(SALES$total, basic = FALSE, norm = TRUE), digits= 3))
sink()
```
...
```{r}
boxplot(total, data=SALES, main= "SALES Questions ",
...
```{r}
SALEScorr.df <- select(SALES, Q07:Q32)
```
...
```{r}
SALEScorr <- cor(SALEScorr.df[sapply(SALEScorr.df, is.numeric)], use='pairwise')
```

Saving the correlation output
```{r}
sink("SALESCorr.doc")
print(correlate(SALEScorr.df, test = TRUE))
sink()
```
...
```{r}
SALESfit <- cfa(latents.model_s,data = SALES, ordered= TRUE, estimator = "DWLS") #ordered = true means that the variables are ordered categoricals
...
RMSEA                                          0.082
The robust analysis produced an even worse fit
...
```{r}
fit <- cfa(latents.model_s, data = SALES, se = "robust", test = "Satorra-Bentler") 
...
#Polychoric correlations and DWLS estimator
```{r}
SALES_polychor <- polychor(SALEScorr.df, std.err = TRUE)
```
