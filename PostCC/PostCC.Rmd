...
```
365 observationns are retained
```{r}
...
#Note that there is a discprenacy between pre and post. The pre receive 0.5 for "Not sure", while post receive 0. Make sure to code the data similarly for both tests
...
```{r}
mshapiro.test(PrePostInd$PostScore, PrePostInd$PreScore)
```
...
```{r}
PostCC_filtered$ID <- factor(PostCC_filtered$ID)
```
...
```{r}
 PostCCscores<- select(PostCCscores, -Response, -X)
```


```{r}
PostCC_Wide <- PostCCscores  %>%
pivot_wider(names_from = Question, values_from = value)
```

```{r}
write.csv(PostCC_Wide, "PostCCWide.csv")
```
