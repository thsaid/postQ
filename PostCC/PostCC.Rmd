...
#12/7/2021 updates:Loadig the latest versio of the data
...
```{r}
PrePostInd <- inner_join(PostCC_Individual, PreCC_Individuals, c("ID"))
```
...
```{r}
write.csv(PrePostInd, "PrePostCCInd_12_7_2021.csv")
```
...
#loading the binrary version of the data to check whether it affected (no need for a bi version)
