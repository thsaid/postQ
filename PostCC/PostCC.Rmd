...
#Problem: I nooticed that when coding the data in excel and replacing the values, the values for Q9Post is transferred into numbers instead of characters. 
...
```{r}
PostCC_long <- gather(PostCC, Question, Response, Q08post:Q14Post, factor_key=TRUE)
```
...
```{r}
PostCCscores <- inner_join(PostCC_long, scoring2, c("Response"))
```

```{r}
PostCCscores_left <- left_join(PostCC_long, scoring2, c("Response"))
```
