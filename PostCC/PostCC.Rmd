...
```{r}
PostQ <- read.csv("Post Questionnaire_20200625.csv")
```
...
#12/7/2021
Loadig the latest versio of the data
```{r}
PostQ <- read.csv("PostQ_filteredComplete_12_7_2021.csv", header = T, na.strings=c(""," ","NA"))
```

```{r}
filter(PostQ, duplicated(ID))
```
This shows that none of the IDs is duplciated. 

```{r}
PostCC <- select(PostQ, ID, Q35:Q51)
```

#Recoding the values in the PostCC (from numbers to characters) to prepare for the scorig
Strogly Agree = 5
Agree = 4
Not Sure = 3
Disagree = 2
...
Note you have to do all the recodings in one command, otherwise, the last code will replace the older ones. 
```{r}
PostCC_filtered <-PostCC %>% 
  mutate(
    Q36 = recode_if(Q36, Q36 == "2", "2" = "Disagree"), Q36 = recode_if(Q36, Q36 == "4", "4" = "Agree"),Q36 = recode_if(Q36, Q36 == "3", "3" = "Not Sure"), Q41 = recode_if(Q41, Q41 == "2", "2" = "Disagree"), Q41 = recode_if(Q41, Q41 == "4", "4" = "Agree"),Q41 = recode_if(Q41, Q41 == "3", "3" = "Not Sure"), Q47 = recode_if(Q47, Q47 == "I am 3", "I am 3" = "I am not sure"),Q48 = recode_if(Q48, Q48 == "I am 3", "I am 3" = "I am not sure"))
```

#Recodig Q14 to avoid confusion with other 

```{r}
PostCC_filtered <-PostCC_filtered %>% 
  mutate(
Q14post = recode_if(Q14post, Q14post == "Disagree", "Disagree" = "Disagree14"),Q14post = recode_if(Q14post, Q14post == "Agree", "Agree" = "Agree14"))
```

```{r}
write.csv(PostCC_filtered,"PostCCfilteredComplete_12_7_2021.csv")
```

Saving Individual postscores
```{r}
PostCC_Individual <- select(PostQ, ID,SC0)
PostCC_Individual<- PostCC_Individual %>%
  rename(PostScore = SC0) 
```

```{r}
write.csv(PostCC_Individual, "PostCC_Individual_12_7_2021.csv")
```

```{r}
hist(PostCC_Individual$PostScore)
```

```{r}
PreCC_Individuals <- read.csv("PreCC_Individuals_12_7_2021.csv")
```

#Joining both tables to compare the scores of individuals

```{r}
PrePostInd <- inner_join(PostCC, PreCC_Individuals, c("ID"))
```
365 observationns are retained from this 
```{r}
PrePostInd <- select(PrePostInd, -X)
```

```{r}
write.csv(PrePostInd, "PrePostCCInd_20_10_2021.csv")
```
...
```{r}
plot(PrePostInd$ID, y= c(PrePostInd$PostScore, PrePostInd$PreScore))
```

#Comparision of individual scores (pre/post)
```{r}
boxplot(PrePostInd$PreScore, PrePostInd$PostScore)
main="Comparing Pre and Post means for total scores"))
```

#Note that there is a discprenacy between pre and post. The pre receive 0.5 for "Not sure", while post receive 0. Make sure to code the data similarly for both tests (solved on 12/7/2021)

```{r}
plot( PrePostInd$PreScore,PrePostInd$PostScore,)
abline(a= 0, b=1)
```

The points above the line shows the improvement post the intervention, while those below the line show no improvement. 
...
```{r}
mvn(PrePostInd, subset = "PostScore", mvnTest = c("mardia"))
```

Something wrong with the code,.will have to try to quit and restart the session again.
...
```{r}
PrePostInd %>%
shapiro_test(PostScore, PreScore )
```
The shapiro test shows that the data is NOT normally disribtubed, because the p value is very low. 
PostScore	0.9405544	2.137345e-10		
PreScore	0.9467932	1.102723e-09	

#qq plots

```{r}
library(ggpubr)
ggqqplot(PrePostInd$PostScore)
ggqqplot(PrePostInd$PreScore)
```



#Assessing normaility of the indivual question scores 

```{r}
PrePost_mean %>%
shapiro_test(PreValue, Post.Value)
```


```{r}
hist(PrePost_mean$PreValue)
hist(PrePost_mean$Post.Value)
```

It appears that the prescores of the questions are not normally distributed, however, the postscores are normally distributed
...
```{r}
 PostCC_filtered <- rename(PostCC_filtered, Q08post=Q35, Q09post= Q36, Q10post= Q37, Q11post= Q38, Q12post = Q39, Q13post= Q40, Q14post = Q41, Q15post = Q42, Q16post= Q43, Q17post = Q44, Q18post = Q45, Q19post = Q46, Q20post = Q47, Q21post = Q48, Q22post = Q49, Q24post = Q51)
```
...
Arrange coloumns alphabetically
```{r}
PostCC <- PostCC[,order(colnames(PostCC))]
...
```{r}
PostCC_long <- gather(PostCC_filtered, Question, Response, Q08post:Q24post, factor_key=TRUE)
```
...
```{r}
scoring <- read.csv("Scoring_CC2_copy.csv")
```


```{r}
PostCCscores <- inner_join(PostCC_long, scoring, c("Response"))
```

Exporting the PostCC scores

```{r}
write.csv(PostCCscores, "PostCCscores_20210712.csv")
```

```{r}
PreCCscores <- read.csv("PreCC_Scores_12_7_2021.csv")
```

#Move this down after converting to wide format
...
Renaming the variables to join both tables
```{r}
PostCC_renamed <- rename(PostCC_filtered, Q08=Q35, Q09= Q36, Q10= Q37, Q11= Q38, Q12 = Q39, Q13= Q40, Q14 = Q41, Q15 = Q42, Q16= Q43, Q17 = Q44, Q18 = Q45, Q19 = Q46, Q20 = Q47, Q21 = Q48, Q22= Q49, Q24 = Q51)
```


```{r}
PostCC_long2 <- gather(PostCC, Question, Response, Q08:Q24, factor_key=TRUE)
```


#Testing the normality of the disrtibution


Chekcing normality of mean scores
...
#Note: Recheck the data, since the max. pre score is 25, while the max post score is 16. There is something not correct!
...
Removing respnses from PostCCScores

```{r}
 PostCCscores_response<- select(PostCCscores, -Response)
```


```{r}
PostCC_Wide <- PostCCscores_response  %>%
pivot_wider(names_from = Question, values_from = PostValue)
```

```{r}
write.csv(PostCC_Wide, "PostCCWide_12_7_2021.csv")
```

loading preCC wide

```{r}
PreCC_Wide <- read.csv("PreCC_Wide_12_7_2021.csv")
```

Merging both wide versions

```{r}
PrePostWide <- merge(PostCC_Wide, PreCC_Wide, by= "ID")
```

Arrange coloumns alphabetically 
```{r}
PrePostWide[,order(colnames(PrePostWide))]
```
Did not work

```{r}
describe(PrePostWide)
```

Exproting the latest version of prepoost (14/7)

```{r}
write.csv(PrePostWide, "PrePostWide_14_7_2021.csv")
```
...
```{r}
write.csv(PrePostWide, "PrePostCC_wide_raw.csv")
```
...
For analysis, removing ID and X

```{r}
PrePostCC_Wide <- dplyr:: select(PrePostCC_Wide, -ID, -X)
```
...
```{r}
PrePostCC_Frequency <- ExpCustomStat(PrePostCC_Wide,Cvar = c("Q08","Q9","Q10", "Q11", "Q12", "Q13", "Q14", "Q15", "Q16", "Q17", "Q18", "Q19", "Q20", "Q21", "Q22", "Q24"), gpby= FALSE)
```
...
```{r}
PreCC_Cor <-rcorr(as.matrix(PreCCmatrix), type = c("spearman"))
PreCC_rho = PreCC_Cor$r
```

```{r}
cor.plot(PreCC_rho, main = "PreCC_Spearman Correlation 20210714")
```


I am not sure if it is appropriate to have a different sclae for questions 9 & 14, so I will change it to zeros & 1s and will see how this affect the correlations. 

This was done for questions: 9, 14, 20 & 21

#loading the binrary version of the data to check whether it affected 

```{r}
PostCC_bi <- read.csv("PostCCWide_bi.csv")

PostCC_bi_matrix <- dplyr::select(PostCC_bi, -ID, -X)
...
```{r}
PostCC_CorBi <-rcorr(as.matrix(PostCC_bi_matrix), type = c("spearman"))
PostCC_rho = PostCC_CorBi$r
```



```{r}
cor.plot(PostCC_rho, main = "PostCC_Bi_Spearman Correlation 20210714")
```
Trying tetrachoric & comparing it to spearman

```{r}
PostCC_tet <- tetrachoric(PostCC_bi_matrix)
```

```{r}
cor.plot(PostCC_tet$rho)
```

#EFA (note the below analysis with done after converting all the questions to 0s & 1s; thus the name Bi = binary was called for these models)


```{r}
fa.parallel(PreCC_rho, n.obs=413, fm="pa", fa="fa", main = "Scree Plot") #You need to insert the correlation matrix not the raw matrix itself
```
...
```{r}
PostCCBi_model_4 = fa(PreCC_rho, nfactor=4, n.obs= 413, fm="uls", rotate = "none") #I can supply the correlation matrix directly, or provide the data frame and decide which cor type to be done: 'tet' stands for tetrachoric
```
...
```{r}
fa.diagram(PostCCBi_model_4)
```
 

 The scores seem to be very good: residuals (RMSR) is  0.03 , Tucker Lewis Index of factoring reliability =  0.928
RMSEA index =  0.043  and the 90 % confidence intervals are  0.029 0.056
BIC =  -264.01

Note: The thrid and 4th factor had negative scores

Trying out 2 factor models and comparing the scores 
...
```{r}
PostCCBi_model_2 = fa(PreCC_rho, nfactor=2, n.obs= 413, fm="uls", rotate = "none")
PostCCBi_model_2 
```
The scores decreased in this case (Tucker Lewis Index of factoring reliability =  0.886
RMSEA index =  0.054)

```{r}
fa.diagram(PostCCBi_model_2)
```

```{r}
factor.congruence(list(PostCCBi_model_2,PostCCBi_model_4))
```

 
#Examining the polymodel (where the questions are graded in 0.5 & 1)


```{r}
PostCC_Cor <-cor(PostCC_Wide[sapply(PostCC_Wide, is.numeric)], use='pairwise', method = c("spearman"))
```


```{r}
cor.plot(PostCC_rho, main = "PostCC_poly_Spearman Correlation")
```

Trying out polychor

```{r}
PostCC_polyMatrix <- select(PostCC_Wide, -ID)
```


```{r}
PostCCPolyCor <- polychoric(PostCC_polyMatrix)
par(mar = c(2, 3, 3, 2))
cor.plot(PostCCPolyCor$rho, main = "PostCC_Polychoric Correlations")
image(as.matrix(leg),col=cx,axes=T)
```


The results of the bi matrix looks better. Generally, spearman correlations produce more moderate correlations than polychoric The poluchoric model resulted in negative correlations. 

#EFA for the polymodel

```{r}
PostCCpoly_model_2 = fa(PostCC_Cor, nfactor=2, n.obs= 413, fm="uls", rotate = "none")
PostCCpoly_model_2 

```
Tucker Lewis Index of factoring reliability =  0.856
RMSEA index =  0.07  and the 90 % confidence intervals are  0.061 0.08

```{r}
fa.diagram(PostCCpoly_model_2)
```


Trying 4 factor model

```{r}
PostCCpoly_model_4 = fa(PostCC_Cor, nfactor=4, n.obs= 413, fm="uls", rotate = "none")
PostCCpoly_model_4
```

#Redoing the file again on the 20th of Oct Mering pre and post

```{r}
PostCC <- read.csv("PostCC_Ind20_10_21.csv")
```
