...
```{r}
PostQ <- read.csv("postQ_unique_complete.csv")
```
...
```{r}
write.csv(PostCCscores, "PostCCscores_20210705.csv")
```
...
```{r}
PostCCscoresleft <- left_join(PostCC_long, scoring, c("Response"))
```
...
```{r}
hist(Indvidual_PostCCScores$PostScore)
```
...
```{r}
hist(PrePostInd$PostScore)
hist(PrePostInd$PreScore)
```
...
```
The shapiro test shows that the data is normally disribtubed, because the p value is very low. 
