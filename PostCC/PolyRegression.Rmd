...
#FM on Age 
cubic relation seemed significant than other models  
```{r}
...
It seems that the modelR package adds the residuals in the unstandardised way compared to the std.res command
...
#SS on Age
Polynomial regression indicated significant linear relation with age 
...
#SS on age showed to be a quadratic function instead
...
```{r}
write.csv(FullData_Z, "FullDataRes_2610.csv")
```

#Residualizng post score on pre score

```{r}
Post_adj <-lm(PostScore ~ PreScore + Age , data = FullData)
Gain_stdres <- rstandard(Post_adj)
FullData_Z2011 <- cbind(FullData_Z,Gain_stdres)
```

```{r}
FullData_Z <- FullData_Z2011
```

```{r}
write.csv(FullData_Z2011, "FullDataRes_2011.csv")
```
