...
```{r}
write.csv(CC_SALES_PALS_SS_FM, "CC_SALES_PALS_SS_FM_20211021.csv")
```

```{r}
CB_means <- read.csv("CB_means_15092021.csv")
```

```{r}
CB_means<- rename(CB_means, ID = subject_id)
```


```{r}
FullData2310 <- merge(CC_SALES_PALS_SS_FM, CB_means, by = "ID")
```
...
```{r}
FullData2310_Demo <- merge(FullData2310, Demog, by = "ID")
```


```{r}
FullData2310_Z <- merge(FullData_Z, FullData2310, by = "ID")
```

```{r}
write.csv(FullData2310_Z, "FullData2310.csv")
```
...
Cormat <- cor(CC_SALES_PALS_SS_FM[sapply(CC_SALES_PALS_SS_FM, is.numeric)], use='pairwise')
```
