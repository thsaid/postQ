...
```{r}
PrePostCC <-read.csv("PrePostCCIndGain_25_7_2021.csv")
```

```{r}
SALES_scores <- read.csv("SALES_Factors.csv")
```
...
```{r}
CC_SALES <- merge(PrePostCC, SALES_scores, by = "ID")
```

```{r}
CC_SALES <- dplyr::select(CC_SALES, -X, -Picture, -Color)
```

```{r}
write.csv(CC_SALES, "CC_SALES_270721.csv")
```
...
```{r}
SALESmodel3 <- lm(PostScore ~ SelfEfficacy + TaskValue + MasteryGoals + PreScore, data = CC_SALES)
summary(SALESmodel3)
```
...
```{r}
write.csv(PALS_Factors, "PALS_Factors_renamed.csv")
```

Renaming the common vairables
...
```{r}
CC_SALES_PALS <- dplyr::select(CC_SALES_PALS, -X, -X.1, -Gender.x)
```
