...
Recoding values for Q09Post (those values were coded by error in the excel sheet, so the values were replaced to numbers. Resotring them again here to avoid recoding in excel).
...
Recoding values for Q14Post (Agree14 was coded to differentiate it from the Agree in Q9.)

```{r}
PostCC2 <-PostCC2 %>% mutate (Q14PPost = case_when(
PostCC$Q14post == 3 ~ c("Not Sure"),
PostCC$Q14post == 4 ~ c("Agree14"),
PostCC$Q14post == 2 ~ c("Disagree14")))
```

```{r}
PostCC2 %>% 
  mutate(Q144Post = case_when(PostCC$Q14post == 3 ~ c("Not Sure"),
PostCC$Q14post == 4 ~ c("Agree14"),
PostCC$Q14post == 2 ~ c("Disagree14")))
...
```{r}
PostCC <-select(PostCC2, ID:Q14Post, -Q09post, -Q14post)
```
...
#Merging data from pre and post 
```{r}
PrePostCC_Wide2 <- merge(PreCC_Wide,PostCC_Wide)
```
...
```{r}
write.csv(CompDescriptives, "CompAnalysisCC.csv")
```
...
```{r}
PrePostGPCM <- gpcm(PrePostCC_Wide,constraint = "1PL" ,IRT.param = TRUE)
PrePostGPCM
```
