...
=======
There is a missing question: It is important to me that I improve my science skills.

Q25 One of my goals is to master new science skills ()
>>>>>>> Stashed changes
#Loading Libraries
...
=======
imputed_Data$imp$Q25 #these command shows the imputed data for Q25 for all the versions (datasets)
```
...
```{r}
total = rowSums(Filter(is.numeric, SALES))
```
...
It appears that the score distribution is not normal, there is a left tail and the score is skewed. Kurtosis is 6.6 and 

Q-plot to compare the distribution of the sample vs. predicted

```{r}
qqplot.scores <- qplot(sample = SALES$total, stat="qq")
```
